  ...
  import { Building2, MapPin, Clock, ArrowUpRight, CheckCircle2, ChevronLeft, Share2, Banknote } from "lucide-react";
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6 items-start">
              <div className="flex gap-6 items-center">
                <div className="w-24 h-24 bg-white rounded-2xl flex items-center justify-center p-2 shadow-lg">
                  {job.company_logo ? <img src={job.company_logo} className="w-full h-full object-contain" /> : <Building2 className="text-slate-400" />}
                </div>
                <div>
              ...
              <div className="flex gap-3">
                <button className="p-3 bg-white/10 rounded-xl hover:bg-white/20 transition-all"><Share2 className="text-white" size={20}/></button>
              ...
              <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-200/60">
                ...
                <div className="flex flex-wrap gap-3">
                  {job.skills?.map((skill, i) => (
                    <span key={i} className="px-4 py-2 bg-slate-50 border border-slate-200 text-slate-700 font-semibold rounded-lg text-sm">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200/60">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between py-3 border-b border-slate-50">
                    ...
                    <span className="font-bold text-brand-dark">€{job.salary_min/1000}k - €{job.salary_max/1000}k</span>
                  </div>
                  <div className="flex justify-between py-3 border-b border-slate-50">
                    <span className="text-slate-500 text-sm">Posted</span>
                    <span className="font-bold text-brand-dark">{new Date(job.created_at).toLocaleDateString()}</span>
                  </div>
                  <div className="flex justify-between py-3 border-b border-slate-50">
                    <span className="text-slate-500 text-sm">Experience</span>
                    <span className="font-bold text-brand-dark capitalize">{job.experience_level}</span>
                  </div>
                  <div className="flex justify-between py-3">
                    <span className="text-slate-500 text-sm">Industry</span>
                    <span className="font-bold text-brand-dark">{job.industry || 'Tech'}</span>
                  </div>
                </div>
              </div>
            </div>
