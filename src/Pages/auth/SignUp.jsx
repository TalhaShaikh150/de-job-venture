...
function SignUp() {
  ...
  async function handleSignUp(formData) {
    try {
      ...
      if (error.message.includes("already")) {
        setFormMessage("An account with this email already exists. Please log in.");
        reset();
      } else {
  ...
  // Helper to check if the message is a success message
  const isSuccess = formMessage === "Account created. Check your email to confirm.";
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form className="space-y-5" onSubmit={handleSubmit(handleSignUp)}>
        ...
        {formMessage && (
          <p className={`text-sm text-center font-medium py-2 rounded-lg border 
            ${isSuccess 
      ...
      <div className="mt-10 text-center border-t border-slate-100 pt-6">
        <p className="text-slate-500">
          ...
          <Link to="/login" className="text-brand-dark font-bold hover:underline">Log In</Link>
        </p>
      </div>
    </div>
  );
